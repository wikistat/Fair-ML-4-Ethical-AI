...
CUAE <- function(X,S,Y,P,alpha=0.05){
  ...
  q_1<-sum(X[,S]*(1-X[,Y])*X[,P])/n #estimated P(g(X)=1, Y=0, S=1)
  q_0<-sum((1-X[,S])*(1-X[,Y])*X[,P])/n #estimated P(g(X)=1, Y=0, S=0)
  s_1<-sum(X[,S]*X[,P])/n #estimated P(g(X)=1, S=1)
  s_0<-sum((1-X[,S])*X[,P])/n #estimated P(g(X)=1, S=0)
  ...
  ###
  p_1<-sum(X[,S]*X[,Y]*(1-X[,P]))/n #estimated P(g(X)=0, Y=1, S=1)
  p_0<-sum((1-X[,S])*X[,Y]*(1-X[,P]))/n #estimated P(g(X)=0, Y=1, S=0)
  r_1<-sum(X[,S]*(1-X[,P]))/n #estimated P(g(X)=0, S=1)
  r_0<-sum((1-X[,S])*(1-X[,P]))/n #estimated P(g(X)=0, S=0)
  ...
  CUAE_CI<-as.data.frame(rbind(c(lower_limA1,TnA1,upper_limA1),c(lower_limA2,TnA2,upper_limA2)))
  rownames(CUAE_CI)<-c("Y=0/f(X)=1", "Y=1/f(X)=0")
  names(CUAE_CI)<-c("inf","est_value","sup")
  return(CUAE_CI)
}
